...
(defconst sqf-font-lock-keywords-functions-extensive
  (append sqf-font-lock-keywords-basic
          (list
           `(,(regexp-opt sqf-mode-keywords 'words) . font-lock-function-name-face)))
  "Functions provided with iamthesux/arma-mode")

;;functions - user compiled
(defconst sqf-font-lock-keywords
  (append sqf-font-lock-keywords-functions-extensive
          (list
           '("\\<\\w+_fnc_\\w+\\>" . font-lock-function-name-face)
           '("\\<\\(_\\(?:\\(?:forEachInde\\)?x\\)\\)\\>" . font-lock-variable-name-face)))
  "User compiled functions matching `word'_fnc_`word'")

(defvar sqf-syntax-table
  (let ((st (make-syntax-table)))
    (modify-syntax-entry ?_ "w" st)
    (modify-syntax-entry ?/ ". 124b" st)
    (modify-syntax-entry ?\n "> b" st)
    (modify-syntax-entry ?* ". 23" st)
    (modify-syntax-entry ?' "\"" st)    ;; strings in ' '
    st)
...
(define-derived-mode sqf-mode prog-mode "SQF"
  "A major mode for editing SQF files."
  :syntax-table sqf-syntax-table
